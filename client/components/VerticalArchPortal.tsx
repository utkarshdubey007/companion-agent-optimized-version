...
const GoldenSparkle = ({ index }: { index: number }) => {
  const sparklePositions = [
    // Top row particles
    { x: 12, y: 8, size: 2.5, glow: true },
    { x: 28, y: 6, size: 1.4, glow: false },
    { x: 52, y: 4, size: 1.8, glow: false },
    { x: 68, y: 8, size: 2.2, glow: true },
    { x: 88, y: 12, size: 1.6, glow: false },

    // Upper sides and middle area
    { x: 6, y: 18, size: 1.3, glow: false },
    { x: 94, y: 20, size: 1.7, glow: false },
    { x: 18, y: 25, size: 1.2, glow: false },
    { x: 82, y: 28, size: 1.6, glow: false },
    { x: 8, y: 32, size: 1.9, glow: false },
    { x: 92, y: 35, size: 1.5, glow: false },

    // Around portal area
    { x: 15, y: 45, size: 1.4, glow: false },
    { x: 85, y: 48, size: 1.8, glow: false },
    { x: 22, y: 55, size: 1.3, glow: false },
    { x: 78, y: 58, size: 2.0, glow: true },
    { x: 4, y: 50, size: 1.2, glow: false },
    { x: 96, y: 45, size: 1.5, glow: false },

    // Lower area particles
    { x: 12, y: 68, size: 1.7, glow: false },
    { x: 88, y: 72, size: 1.5, glow: false },
    { x: 25, y: 78, size: 1.6, glow: false },
    { x: 75, y: 82, size: 1.9, glow: false },
    { x: 38, y: 88, size: 1.4, glow: false },
    { x: 62, y: 85, size: 1.7, glow: false },
    { x: 2, y: 75, size: 1.8, glow: false },
    { x: 98, y: 65, size: 1.3, glow: false },

    // Scattered particles
    { x: 35, y: 35, size: 1.0, glow: false },
    { x: 65, y: 42, size: 1.1, glow: false },
    { x: 45, y: 55, size: 1.2, glow: false },
    { x: 55, y: 38, size: 1.0, glow: false },
  ];
  ...
  return (
    <div className={`relative w-full h-screen overflow-hidden ${className}`}>
      ...
      >
        ...
        {/* Two four-pointed stars exactly positioned like in reference */}
        <motion.div
          className="absolute"
        ...
        >
          <div className="relative w-5 h-5">
            <div
              className="absolute w-5 h-0.5 bg-yellow-300 left-0 top-1/2 transform -translate-y-1/2"
              style={{ boxShadow: "0 0 15px rgba(255, 215, 0, 1)" }}
            />
            <div
              className="absolute w-0.5 h-5 bg-yellow-300 left-1/2 top-0 transform -translate-x-1/2"
              style={{ boxShadow: "0 0 15px rgba(255, 215, 0, 1)" }}
            />
            <div
              className="absolute w-2 h-2 bg-yellow-100 rounded-full left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2"
        ...
        >
          <div className="relative w-4 h-4">
            <div
              className="absolute w-4 h-0.5 bg-yellow-300 left-0 top-1/2 transform -translate-y-1/2"
              style={{ boxShadow: "0 0 12px rgba(255, 215, 0, 0.9)" }}
            />
            <div
              className="absolute w-0.5 h-4 bg-yellow-300 left-1/2 top-0 transform -translate-x-1/2"
              style={{ boxShadow: "0 0 12px rgba(255, 215, 0, 0.9)" }}
            />
            <div
              className="absolute w-1.5 h-1.5 bg-yellow-100 rounded-full left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2"
